...
const Main = ({ match, markets }: Props) => {  
  ...
  return (
    <div className=" flex justify-between px-4 md:px-0 dashed-border py-2">
      <div className="flex flex-col justify-center gap-1 w-1/2 md:w-auto md:py-1.5 cursor-pointer" onClick={ () => moreMarkets(match.id) }> 
          <div className='text-xs'>
            <span className=' text-gray-300'>
              {/* <Moment format='hh:ss • DD/MM'>{ moment(`${match.scheduled}`) }</Moment> */} { moment(match.scheduled).format('D/MM HH:mm') }
            </span>  
            <Link href={`/single-match/${match.id}`} className={ `ml-1 text-yellow cursor-pointer ${select ? ' text-orange-300' : ''}` } onClick={ () => moreMarkets(match.id) }>
